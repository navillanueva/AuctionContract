...
describe("AuctionV2", async function () {
  ...
  describe("Place Bid", async function () {
    ...
    it("reverts if you are the owner", async function () {
      await expect(
        auctionv2.placeBid(auctionItems[0])
      ).to.be.revertedWithCustomError(auctionv2, "AuctionV2__Owner");
    });
    ...
    it("reverts if the bid is not higher than the highest bid", async function () {
      await auctionv2
        .connect(bidder1)
        .placeBid(auctionItems[0], { value: sendValue3 });
      await expect(
        auctionv2
          .connect(bidder2)
          .placeBid(auctionItems[0], { value: sendValue2 })
    ...
    it("returns previous highest bid to its bidder", async function () {
      ...
      let startingBidderBalance = await auctionv2.provider.getBalance(
        bidder1.address
      );
      ...
      let endingBidderBalance = await auctionv2.provider.getBalance(
        bidder1.address
      );
  ...
  describe("Get Highest Bidders", async function () {
    ...
    beforeEach(async function () {
      ...
      await auctionv2
        .connect(bidder2)
        .placeBid(auctionItems[1], { value: sendValue4 });
    });
