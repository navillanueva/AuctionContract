...
describe("AuctionV1", async function () {
  ...
  describe("Place Bid", async function () {
    ...
    it("reverts if you are the owner", async function () {
      await expect(
        auctionv1.placeBid(auctionItems[0])
      ).to.be.revertedWithCustomError(auctionv1, "AuctionV1__Owner");
    });
    ...
    it("returns previous highest bid to its bidder", async function () {
      ...
      let startingBidderBalance = await auctionv1.provider.getBalance(
        bidder1.address
      );
      ...
      let endingBidderBalance = await auctionv1.provider.getBalance(
        bidder1.address
      );
